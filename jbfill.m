function[fillhandle,msg]=jbfill(xpoints,upper,lower,color,edge,add,transparency,labelName)
...
%     grid on
%     legend('Datr')
if nargin<8;labelName="unknown";end
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    fillhandle=fill(xpoints,filled,color,'DisplayName',labelName);%plot the data
    set(fillhandle,'EdgeColor',edge,'FaceAlpha',transparency,'EdgeAlpha',0.8);%set edge color
    if strcmp(labelName,"unknown")
        set(get(get(fillhandle,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
    end
    if add
        hold off
    end
