...
% INITIAL CONDITION
parameters.alpha = 2e5; % absorption coefficient at the laser wavelength cm^-1
...
parametersContinuous.generationRate =   @(z,t,modelParameterZ) modelParameterZ.cw_flux*modelParameterZ.alpha*exp(-modelParameterZ.alpha*z);
...
tau_dyn = -1/p(1);
semilogy(parameters.tvec,deltan,'--','DisplayName',sprintf("With transport$\\tau_{dyn}$ = %0.2fns",tau_dyn*1e9))
hold on;
...
dndt = diff(deltan)./diff(parameters.tvec');
figure(78);
loglog(PLsignal(1:end-1)/parameters.thickness,-dndt,'DisplayName','with space');
hold on;
xlabel("$I_{PL}$")
ylabel("-dn/dt")
